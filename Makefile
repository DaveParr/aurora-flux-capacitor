...
build:
ifndef PROJECT
	$(error PROJECT is not set. Usage: make build PROJECT=flux-capacitor)
endif
	$(MAKE) -C $(PROJECT) TARGET=$(PROJECT)-$(FIRMWARE_VERSION)

flash:
ifndef PROJECT
	$(error PROJECT is not set. Usage: make flash PROJECT=flux-capacitor MOUNT=/media/dave/AURORA)
endif
ifndef MOUNT
	$(error MOUNT is not set. Usage: make flash PROJECT=flux-capacitor MOUNT=/media/dave/AURORA)
endif
